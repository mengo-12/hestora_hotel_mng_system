        ...
        <div className="flex flex-col gap-6 mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow text-center">
                    <p className="text-gray-500 dark:text-gray-300 text-sm">Total Rooms</p>
                    <p className="text-xl font-bold">{totalRooms}</p>
                </div>
                <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow text-center">
                    <p className="text-gray-500 dark:text-gray-300 text-sm">Vacant</p>
                    <p className="text-xl font-bold text-green-500">{statusCounts.VACANT}</p>
                </div>
                <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow text-center">
                    <p className="text-gray-500 dark:text-gray-300 text-sm">Occupied</p>
                    <p className="text-xl font-bold text-red-500">{statusCounts.OCCUPIED}</p>
                </div>
                <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow text-center">
                    <p className="text-gray-500 dark:text-gray-300 text-sm">Cleaning</p>
                    <p className="text-xl font-bold text-yellow-500">{statusCounts.CLEANING}</p>
                </div>
                <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow text-center">
                    <p className="text-gray-500 dark:text-gray-300 text-sm">Maintenance</p>
                    <p className="text-xl font-bold text-blue-500">{statusCounts.MAINTENANCE}</p>
                </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-3 flex-wrap md:flex-nowrap items-end bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
                ...
                <div className="flex flex-col w-full md:w-1/4">
                    <label className="mb-1 text-gray-500 dark:text-gray-300 text-sm font-medium">Search Room</label>
                    <input
                        type="text"
                        placeholder="Search..."
                        value={searchTerm}
                ...
                {/* Priority */}
                <div className="flex flex-col w-full md:w-1/5">
                    <label className="mb-1 text-gray-500 dark:text-gray-300 text-sm font-medium">Priority</label>
                    <select
                ...
                <div key={floor} className="mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {groupedByFloor[floor].map(room => (
                            <div
                                key={room.id}
                                className="p-4 rounded-2xl shadow-lg dark:bg-gray-700 bg-white text-black dark:text-white transition transform hover:scale-105 hover:shadow-xl flex flex-col"
                            >
                                {/* Header */}
                                <div className="flex justify-between items-start mb-3">
                                    <div>
                                        <h3 className="text-lg font-semibold">Room {room.number}</h3>
                                        <p className="text-sm text-gray-500 dark:text-gray-300">Floor {room.floor || "N/A"}</p>
                                    </div>
                                    <span className={`px-2 py-1 text-xs rounded ${statusColors[room.status] || "bg-gray-400 text-white"}`}>
                                        {room.status}
                                    </span>
                                </div>
                                ...
                                <div className="mb-3">
                                    ...
                                    {room.housekeepingTasks.length === 0 ? (
                                        ...
                                        <ul className="space-y-1">
                                            {room.housekeepingTasks.map(task => (
                                                <li key={task.id} className="bg-gray-100 dark:bg-gray-600 rounded p-2 text-sm flex justify-between items-center">
                                                    <span>{task.type} ({task.priority}) - {task.status}</span>
                                                    {task.status === "Open" && canManageTasks && (
                                                        <button
                                ...
                                <div className="flex flex-wrap gap-2 mt-auto">
                                    {canModifyRoom && (
                                        <>
                                            <button onClick={() => updateRoomStatus(room.id, "CLEANING")} className="text-xs px-2 py-1 rounded bg-yellow-500 text-white hover:bg-yellow-600">Cleaning</button>
                                            <button onClick={() => updateRoomStatus(room.id, "VACANT")} className="text-xs px-2 py-1 rounded bg-green-500 text-white hover:bg-green-600">Vacant</button>
                                            <button onClick={() => updateRoomStatus(room.id, "MAINTENANCE")} className="text-xs px-2 py-1 rounded bg-blue-500 text-white hover:bg-blue-600">Maintenance</button>
                                        </>
                                    )}
                                    {canManageTasks && (
                                        <button
