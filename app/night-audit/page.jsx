...
export default function NightAuditPage({ initialPropertyId, initialProperties, darkMode = false }) {
    ...
    const getStepColor = (step) => {
        if (!step) return "bg-gray-200 dark:bg-gray-700";
        if (["CLOSE_DAY", "OPEN_NEW_DAY"].includes(step)) return "bg-red-200 dark:bg-red-700";
        if (step === "POST_ROOM_CHARGES") return "bg-blue-200 dark:bg-blue-800";
        if (step === "UPDATE_RESERVATIONS") return "bg-green-200 dark:bg-green-800";
    ...
    return (
        <div className={`${darkMode?"bg-gray-900 text-white min-h-screen":"bg-white text-black"} p-6`}>
            ...
            <div className="flex flex-wrap gap-4 mb-4 items-center">
                ...
                <button onClick={runAudit} disabled={running} className={`px-4 py-2 rounded ${running?"bg-blue-400":"bg-blue-600 text-white"}`}>{running?"Running...":"Run Night Audit"}</button>
