...
//             // --- حساب Room, Extras, Taxes ---
//             const nights = b.nights || 1;
//             const roomPrice = Number(b.room?.ratePlan?.basePrice || 0);
//             const roomTaxPercent = Number(b.room?.ratePlan?.taxPercent || 0);
//             const roomTax = (roomPrice * roomTaxPercent / 100) * nights;
//             const totalRoom = roomPrice * nights + roomTax;

//             const extrasTotal = (b.folio?.extras || []).reduce((sum, ex) => {
//                 const price = Number(ex.price || 0);
//                 const qty = Number(ex.quantity || 1);
//                 const tax = Number(ex.tax || 0);
//                 return sum + price * qty + ((price * qty * tax) / 100);
//             }, 0);

//             const adjustments = Number(b.folio?.adjustments || 0);
//             const grandTotal = totalRoom + extrasTotal + adjustments;
            
//             doc.text(`Room: ${b.room?.number || "-"}  Guest: ${guestName}  Status: ${b.status}  Type: ${b.room?.roomType?.name || "-"}  Nights: ${nights}`, 14, cursorY);
//             doc.text(`Room: ${totalRoom.toFixed(2)} • Extras: ${extrasTotal.toFixed(2)} • Adjustments: ${adjustments.toFixed(2)} • Grand Total: ${grandTotal.toFixed(2)}`, 14, cursorY + 6);
//             cursorY += 12;
