...
export async function POST(req) {
    try {
        ...
        const { propertyId, folioGuestId, checkIn, checkOut, companyId, specialRequests, rooms = [] } = body;

        if (!propertyId || !folioGuestId || !checkIn || !checkOut || rooms.length === 0) {
            return new Response(JSON.stringify({ error: "Missing required fields" }), { status: 400 });
        }

        // --- 1. إنشاء الفاتورة باسم رئيس المجموعة ---
        const folio = await prisma.folio.create({
            data: {
                propertyId,
                guestId: folioGuestId,
                companyId: companyId || null,
                checkIn: new Date(checkIn),
                checkOut: new Date(checkOut),
                specialRequests: specialRequests || null,
        ...
        // --- 2. إنشاء كل غرفة / نزيل وربطها بالفاتورة ---
        for (const r of rooms) {
            if (!r.roomId || !r.guestId || !r.ratePlanId) continue;

            // تحقق من الغرفة
            const room = await prisma.room.findUnique({ where: { id: r.roomId }, include: { roomType: true } });
            if (!room) throw new Error(`Room not found for guest ${r.guestId}`);
            if (room.status !== "VACANT") throw new Error(`Room not available for guest ${r.guestId}`);

            // إنشاء الحجز
            const booking = await prisma.booking.create({
                data: {
                    propertyId,
                    folioId: folio.id,        // ربط بالفاتورة الرئيسية
                    guestId: r.guestId,       // النزيل الحقيقي
                    roomId: r.roomId,
                    ...
                    companyId: companyId || null,
                    status: "Booked",
                    extras: r.extras?.length > 0 ? {
                        create: r.extras.map(ex => ({
                            name: ex.name,
                            description: ex.description || "",
                            unitPrice: ex.price,
                ...
                include: { guest: true, room: { include: { roomType: true } }, ratePlan: true, extras: true }
            });

            // حساب سعر الغرفة و الضرائب
            let roomPrice = Number(booking.ratePlan?.basePrice || 0);
            let roomTax = roomPrice * (Number(booking.ratePlan?.taxPercent || 0) / 100);
            ...
            await prisma.charge.create({
                data: {
                    folioId: folio.id,
                    code: "ROOM",
                    description: `Room Charge - ${booking.room?.roomType?.name || "Room"}`,
                    amount: roomPrice,
                    tax: roomTax,
                    postedById: currentUserId
                }
            ...
            // إضافة Extras كـ Charges
            for (const ex of booking.extras) {
                const subTotal = ex.unitPrice * ex.quantity;
                const taxAmount = subTotal * (ex.tax / 100);
                await prisma.charge.create({
                    data: {
                        folioId: folio.id,
                        code: "EXTRA",
        ...
        try {
            await fetch("http://localhost:3001/api/broadcast", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ event: "BOOKING_CREATED", data: createdBookings })
            });
            ...
            for (const r of rooms) {
                await fetch("http://localhost:3001/api/broadcast", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ event: "ROOM_STATUS_CHANGED", data: { roomId: r.roomId, newStatus: "Reserved" } })
                });
            }
        } catch (err) {
        ...
        return new Response(JSON.stringify({ folio, bookings: createdBookings }), { status: 201 });
...
export async function PUT(req) {
    try {
        ...
        for (const b of bookings) {
            const { id, roomId, checkIn, checkOut, ratePlanId, adults, children, specialRequests, extras = [] } = b;
            if (!id) continue;

            const existingBooking = await prisma.booking.findUnique({ where: { id } });
            if (!existingBooking) throw new Error(`Booking not found: ${id}`);
            ...
            const updated = await prisma.booking.update({
                ...
                data: {
                    ...
                    ratePlanId: ratePlanId || null,
                    adults: adults ?? existingBooking.adults,
                    children: children ?? existingBooking.children,
                    specialRequests: specialRequests ?? existingBooking.specialRequests
                },
                include: { folio: true, extras: true }
            });

            // حذف Extras القديمة
            await prisma.extra.deleteMany({ where: { bookingId: id } });

            // إضافة Extras الجديدة
            for (const ex of extras) {
            ...
            if (roomId && roomId !== existingBooking.roomId) {
                ...
                if (existingBooking.roomId) {
                    await prisma.room.update({ where: { id: existingBooking.roomId }, data: { status: "VACANT" } });
                }
            }

            updatedBookings.push(await prisma.booking.findUnique({
                where: { id },
                include: { guest: true, room: { include: { roomType: true } }, ratePlan: true, company: true, folio: { include: { charges: true, extras: true } }, extras: true }
            }));
        }
        ...
        console.error("Bulk booking update failed:", err);
        return new Response(JSON.stringify({ error: err.message || "Failed to update bulk bookings" }), { status: 500 });
    }
}
...
export async function DELETE(req) {
    try {
        ...
        if (!bookingIds.length) throw new Error("No booking IDs provided.");
        ...
        for (const id of bookingIds) {
            ...
            // تحديث حالة الغرفة إلى VACANT
            if (booking.roomId) {
                await prisma.room.update({ where: { id: booking.roomId }, data: { status: "VACANT" } });
            }

            // حذف Extras و Charges
            await prisma.extra.deleteMany({ where: { bookingId: id } });
            if (booking.folioId) {
                await prisma.charge.deleteMany({ where: { folioId: booking.folioId } });
            }

            // حذف Folio
            if (booking.folioId) {
                await prisma.folio.delete({ where: { id: booking.folioId } });
            }

            // حذف الحجز
            await prisma.booking.delete({ where: { id } });

            deletedBookings.push({ id, status: "deleted" });
        }
        ...
        console.error("Bulk booking deletion failed:", err);
        return new Response(JSON.stringify({ error: err.message || "Failed to delete bookings" }), { status: 500 });
    }
}