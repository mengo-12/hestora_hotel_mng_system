...
export default function EditCompanyModal({ company, isOpen, onClose, properties, onSaved }) {
    const [name, setName] = useState(company?.name || "");
    const [code, setCode] = useState(company?.code || "");
    const [propertyId, setPropertyId] = useState(company?.propertyId || properties[0]?.id || "");
    ...
    const handleSave = async () => {
        ...
        try {
            const res = await fetch("/api/companies", {
                ...
                body: JSON.stringify({ id: company.id, name, code, propertyId, creditLimit, rateAgreement }),
            });

            const updatedCompany = await res.json();
            if (!res.ok) throw new Error(updatedCompany.error || "Failed to update company");
