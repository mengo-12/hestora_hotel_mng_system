...
// ------------------نسخة جديدة للتجربة اضافة خدمات يدويا---------------------
...
export default function BookingForm({ booking = null }) {
    ...
    const [formData, setFormData] = useState({
        ...
        source: 'Walk-in',
        notes: '',
        extraServices: [], // تشمل خدمات جاهزة + يدوية
    });
    ...
    const [manualServiceQty, setManualServiceQty] = useState(1);
    ...
    useEffect(() => {
        if (formData.roomId && formData.checkIn && formData.checkOut) {
            ...
            const extrasTotal = formData.extraServices.reduce((sum, s) => {
                if (typeof s === 'string') {
                    const service = extraServicesList.find(es => es.id === s);
                    return sum + (service ? service.price : 0);
    ...
    return (
        <div className="w-full max-w-5xl p-6 space-y-6 rounded-lg border shadow-md bg-gray-100 dark:bg-gray-800 border-gray-400 dark:border-gray-700">
            ...
            {activeTab === 'selectGuest' && (
                <form onSubmit={handleSubmit} className="space-y-6">
                    {error && <div className="p-3 border rounded text-red-900 bg-red-200 dark:bg-red-900 dark:text-red-200 dark:border-red-700">{error}</div>}
                    {successMsg && <div className="p-3 border rounded text-green-900 bg-green-200 dark:bg-green-900 dark:text-green-200 dark:border-green-700">{successMsg}</div>}
                    ...
                    {/* خدمات إضافية يدوية فقط */}
                    <div className="mt-4 border-t pt-4">
                        <label className="block mb-1 text-gray-800 dark:text-gray-200 font-medium">الخدمات الإضافية</label>

                        {/* قائمة الخدمات الحالية مع زر حذف */}
                        <div className="flex flex-col gap-2 mb-3">
                            {formData.extraServices.map((s, idx) => (
                                <div key={s.id || idx} className="flex items-center justify-between border px-3 py-1 rounded bg-white dark:bg-gray-700">
                                    <span>{`${s.name} x${s.qty} (+${(s.price * s.qty).toFixed(2)} ر.س)`}</span>
                                    <button
                                        type="button"
                                        ...
                                        onClick={() => {
                                            setFormData(prev => ({
                                                ...prev,
                                                extraServices: prev.extraServices.filter((_, i) => i !== idx)
                                            }));
                                        }}
                                    >
                                        حذف
                        ...
                        <div className="flex gap-2 flex-wrap items-end">
                            <input
                                ...
                                className="px-3 py-2 rounded border border-gray-400 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                            />
                            <input
                                type="number"
                                min="0"
                            ...
                            <input
                                ...
                                min="1"
                                placeholder="الكمية"
                                value={manualServiceQty}
                                onChange={e => setManualServiceQty(e.target.value)}
                                className="px-3 py-2 rounded border border-gray-400 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                            />
                            <button
                                type="button"
                                    ...
                                    {dailyPrices.map((d, idx) => (
                                        <tr key={idx} className="text-gray-900 dark:text-gray-100">
                                            ...
                                            <td className="px-3 py-2 border">
                                                {formData.extraServices.map((s, idx2) => {
                                                    if (typeof s === 'string') {
                                                        const service = extraServicesList.find(es => es.id === s);
                                                        return service ? <div key={idx2}>{service.name} (+{service.price} ر.س)</div> : null;
                                                    } else if (typeof s === 'object') {
                                                        return <div key={idx2}>{s.name} x{s.qty} (+{(s.price * s.qty).toFixed(2)} ر.س)</div>;
                                                    }
                                                })}
                                            </td>
                                            <td className="px-3 py-2 border">{d.discount.toFixed(2)} ر.س</td>
