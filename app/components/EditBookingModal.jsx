...
//                                         </td>
//                                         <td className="border p-1">
//                                             ${((Number(ex.price) * Number(ex.quantity)) + ((Number(ex.price) * Number(ex.quantity) * (Number(ex.tax) || 0)) / 100)).toFixed(2)}
//                                         </td>
...
export default function EditBookingModal({ booking, isOpen, onClose, properties, guests, rooms, companies }) {
    ...
    const handleExtraChange = (idx, field, value) => {
        const updated = [...extras];
        if (["price", "quantity", "tax"].includes(field)) updated[idx][field] = Number(value || 0);
    ...
    // --- حساب الأسعار مثل AddBookingModal ---
    const selectedRatePlanData = ratePlans.find(rp => rp.id === form.ratePlanId) || null;
    ...
    const extrasSubtotal = extras.reduce(
        (sum, ex) => sum + (Number(ex.price || 0) * Number(ex.quantity || 1)),
        0
    );

    const extrasTax = extras.reduce(
        (sum, ex) =>
            sum + ((Number(ex.price || 0) * Number(ex.quantity || 1) * Number(ex.tax || 0)) / 100),
        0
    );
                        ...
                        <table className="w-full border-collapse border border-gray-300">
                            ...
                            <tbody>
                                {extras.map((ex, idx) => {
                                    ...
                                    const exAmount = price * qty;
                                    const exTax = (exAmount * taxPercent) / 100;
                                    const exTotal = exAmount + exTax;
                                    return (
                                        <tr key={ex.tempId}>
                                            <td className="border p-1">
                                                <input type="text" value={ex.name} onChange={e => handleExtraChange(idx, "name", e.target.value)} className="w-full border rounded p-1" />
