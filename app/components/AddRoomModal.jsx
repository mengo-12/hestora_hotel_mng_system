...
'use client';
import { useState, useEffect } from "react";
import { FaBuilding, FaDoorOpen, FaLayerGroup, FaTools, FaStickyNote } from "react-icons/fa";
...
export default function AddRoomModal({ isOpen, onClose, onSaved, userId }) {
    ...
    useEffect(() => {
        if (isOpen) {
            setForm({ propertyId: "", number: "", roomTypeId: "", status: "VACANT", floor: "", notes: "" });
            setError(null);

            fetch("/api/properties")
                .then(res => res.json())
                .then(data => setLocalProperties(Array.isArray(data) ? data : []))
                .catch(err => setLocalProperties([]));

            fetch("/api/roomTypes")
                .then(res => res.json())
                .then(data => setLocalRoomTypes(Array.isArray(data) ? data : []))
                .catch(err => setLocalRoomTypes([]));
        }
    }, [isOpen]);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-lg p-6 animate-fadeIn">
                <h2 className="text-2xl font-bold mb-5 text-gray-800 dark:text-gray-200 flex items-center gap-2">
                    <FaDoorOpen /> Add New Room
                </h2>
                ...
                <form className="space-y-4" onSubmit={handleSubmit}>
                    ...
                    <div className="flex flex-col">
                        ...
                        <select name="propertyId" value={form.propertyId} onChange={handleChange} required
                            ...
                            <option value="" className="text-black dark:text-white">Select Property</option>
                            {localProperties.map(p => <option className="text-gray-60 dark:text-white" key={p.id} value={p.id}>{p.name}</option>)}
                        </select>
                    </div>
                    ...
                    {/* Floor */}
                    <div className="flex flex-col">
                        <label className="text-sm font-medium text-gray-600 dark:text-gray-300">Floor</label>
                        <input type="number" name="floor" value={form.floor} onChange={handleChange}
                            className="w-full p-2 mt-1 border rounded-lg text-gray-600 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-200" />
                    </div>
