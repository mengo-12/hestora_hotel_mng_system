...
// export default function AddGuestModal({ isOpen, onClose, properties, hotelGroups, onSaved }) {
...
//     const [hotelGroupId, setHotelGroupId] = useState(hotelGroups[0]?.id || "");
...
//                     method: "POST",
//                     headers: { "Content-Type": "application/json" },
//                     body: JSON.stringify({ event: "GUEST_CREATED", data }),
//                 });
//             } catch (err) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-8">
                <h2 className="text-3xl font-bold mb-6 text-gray-900 dark:text-white">Add Guest</h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Passport Number , ID Card</label>
                    ...
                    <div>
                        ...
                        <select value={propertyId} onChange={e => setPropertyId(e.target.value)}
                            className="w-full p-3 border border-gray-300 dark:border-gray-700 rounded-lg dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500">
                            {properties.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                        </select>
                    </div>
