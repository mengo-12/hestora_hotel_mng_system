                ...
                <div key={floor} className="mb-6">
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full border-collapse border border-gray-300">
                            ...
                            <tbody>
                                {groupedByFloor[floor].map(r => (
                                    <tr key={r.roomTypeId} className="">
                                        <td className="border p-2 font-semibold">{r.name}</td>
                                        {dates.map(d => {
                                            const cell = r.cells?.[d] ?? { total: 0, sold: 0, stopSell: false };
                                                ...
                                                <td key={d} className={`border p-1 text-center ${bgColor} text-white`}>
                                                    <div>
                                                        <input
                                                            ...
                                                            onChange={e => handleCellChange(r.roomTypeId, d, 'allotment', parseInt(e.target.value))}
                                                        />
                                                        <div className="text-xs">Sold: {cell.sold}</div>
                                                        <button
                                                            onClick={() => toggleStopSell(r.roomTypeId, d)}
