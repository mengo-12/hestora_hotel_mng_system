...
export default function RoomsPage({ session }) {
    const [rooms, setRooms] = useState([]);
    const [selectedRoom, setSelectedRoom] = useState(null);
    const [editRoom, setEditRoom] = useState(null);
    const [showAddModal, setShowAddModal] = useState(false);
    ...
    useEffect(() => {
        ...
        if (filterStatus) filtered = filtered.filter(room => room.status === filterStatus);
        if (filterRoomType) filtered = filtered.filter(room => room.roomTypeId === filterRoomType);
        if (filterProperty) filtered = filtered.filter(room => room.propertyId === filterProperty);
        setFilteredRooms(filtered);
    }, [searchTerm, filterStatus, filterRoomType, filterProperty, rooms]);
